    ...
    public boolean findMyPosition(String symbol) {
        ...
        for (i = 0; i < 3; i++) {
            for (m = 0; m < 3; m++) {
                ...
                if (n > 2) n = 0;
                if (j > 2) j = 0;
                if (Check.CellSym(i, m, symbol) & Check.Cells(i, m, i, n) & Check.CellSym(i, findThirdCell(n, m), "")) {
                    makeDecision(i, (findThirdCell(n, m)));
                    return true;
                }
                if (Check.CellSym(i, m, symbol) & Check.Cells(i, m, j, m) & Check.CellSym(findThirdCell(i, j), m, "")) {
                    makeDecision(findThirdCell(i, j), m);
                    return true;
                }
                if (Check.CellSym(i, m, symbol) & Check.Cells(i, m, j, n) & Check.CellSym(findThirdCell(i, j), findThirdCell(n, m), "")) {
                    makeDecision(findThirdCell(i, j), (findThirdCell(n, m)));
                    return true;
                }
                if ((2 - i == m) & Check.CellSym(i, m, symbol) & Check.Cells(i, m, j, (findThirdCell(n, m))) & Check.CellSym(findThirdCell(i, j), n, "")) {
